...
const showreelVideo = "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4";
...
function App() {
  ...
  useEffect(() => {
    const preloadVideos = async () => {
      await videoPreloader.preloadBatch([showreelVideo]);
      await videoPreloader.preloadBatch(socialVideos.slice(0, 4));
      await videoPreloader.preloadBatch(featuredVideos.slice(0, 4));
      await videoPreloader.preloadBatch(socialVideos.slice(4, 8));
      await videoPreloader.preloadBatch(featuredVideos.slice(4, 8));
      await videoPreloader.preloadBatch(socialVideos.slice(8));
      await videoPreloader.preloadBatch(featuredVideos.slice(8));
    };
